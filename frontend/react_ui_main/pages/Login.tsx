  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isApiSuccess(response)) {
        ...
        // Map role from backend to Role enum (case-insensitive)
        const roleUpper = user.role?.toUpperCase();
        let userRole: Role;
        
        if (roleUpper === 'ADMIN' || roleUpper === 'TEACHER' || roleUpper === 'STUDENT') {
          userRole = roleUpper as Role;
        } else {
      ...
      <div className="max-w-md w-full">
        ...
        <ClayCard className="p-8 space-y-6">
            <div className="text-center space-y-2">
                ...
                <p className="text-sm text-gray-500">Đăng nhập để tiếp tục học tập và làm bài</p>
            </div>
