...
import { LogOut, LayoutDashboard, ShieldCheck, PieChart } from 'lucide-react';
    ...
    <div className="min-h-screen flex bg-gray-100 font-sans text-gray-800">
      ...
      <aside className="hidden md:flex flex-col w-64 fixed inset-y-0 left-0 bg-gray-100 border-r border-gray-200 z-10">
        ...
        <nav className="flex-1 px-4 space-y-4 mt-4">
          ...
          {role === Role.TEACHER && <SidebarItem icon={<ShieldCheck size={20} />} label="Proctoring" />}
          {role === Role.ADMIN && <SidebarItem icon={<PieChart size={20} />} label="Reports" />}
        </nav>
...
const SidebarItem = ({ icon, label, active = false }: { icon: React.ReactNode, label: string, active?: boolean }) => (
  <div className={`
    ...
    ${active 
      ? 'bg-gray-100 text-primary-600 shadow-clay' 
